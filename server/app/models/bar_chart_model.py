...
class AxisPointer(BaseModel):
  type: str = Field(default="shadow", description="Axis pointer type")
...
class ItemStyle(BaseModel):
  color: str = Field(description="Color for specific data item")


class DataItem(BaseModel):
  value: Union[int, float] = Field(description="Data value")
  itemStyle: Optional[ItemStyle] = Field(
    default=None, description="Custom styling for this data item"
  )


class SeriesItem(BaseModel):
  name: str = Field(default=None, description="Series name")
  data: List[Union[int, float, DataItem]] = Field(default_factory=list, description="Series data")
  type: str = Field(default="bar", description="Chart type")
  barWidth: str = Field(default="60%", description="Bar width as percentage or absolute value")
  ...
  def validate_type(cls, v):
    ...
    valid_types = ["bar"]
    cleaned_type = str(v).lower().strip()
    return cleaned_type if cleaned_type in valid_types else "bar"
  ...
  def clean_data(cls, v):
    ...
    for item in v:
      if item is None:
        ...
        # Handle object with value and itemStyle
        try:
          data_item = DataItem(**item)
          cleaned_data.append(data_item)
        except Exception:
          # If validation fails, just use the value or default to 0
  ...
  @field_validator("barWidth", mode="before")
  @classmethod
  def validate_bar_width(cls, v):
    if v is None:
      return "60%"
    return str(v)
